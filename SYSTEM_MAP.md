...
* **Scoring & Klasifikasi**: `refreshScores()` ➔ `recomputeProductStats()` (Agregasi metrik AI-Emulator: salesConsistency, conversionEfficiency, bestDays/Hours; views decayed, sales/GMV tanpa decay) ➔ `scoreTOPSIS()` (dengan Log-Dampening Math.log1p) / `scoreBenchmark()` ➔ `classifyP()` (WINNING dengan 4 jalur cerdas: Volume, Konsistensi, Efisiensi, Virality) ➔ Urutkan `S.products` ➔ `save()`
* **Generate Jadwal**: Klik Generate ➔ `genSched()` ([js/07-jadwal.js](file:///Users/mhmdrzki/Documents/affiliate-manajemen/js/07-jadwal.js)) ➔ `computeDynamicSlots()` & `computeDayMultiplier()` ➔ Affinity-Based `pickWithCooldown()` (+40% match hari, +60% match jam) ➔ `buildSlotScript()` ➔ `renderSchedOutput()`
